...
#   • PCA / GAN with automatic retrain‑skip (detects input changes)
#   • post‑modern mathematical art (only from PCA images)
#   • FFT enhancement + duotone + fluid motion warp for harsh, dynamic output
# =============================================================================
...
const MOTION_STRENGTH    = 0.12   # how much fluid warp to apply
...
# =============================================================================
# 5. Generate classical styles (always run)
# =============================================================================
...
function duotone(gray_img, dark, light)
    ...
    for i in 1:h, j in 1:w
        t = clamp(gray_img[i,j], 0.0, 1.0)
        t = t^0.75
        out[i,j] = RGB(dark.r*(1-t) + light.r*t,
                       dark.g*(1-t) + light.g*t,
                       dark.b*(1-t) + light.b*t)
    end
    out
end
...
function add_grain(img, intensity=0.015)
    ...
    clamp.(img .+ noise, 0.0, 1.0)
end

# =============================================================================
# 10. Image‑conditioned fractal generators (grayscale)
# =============================================================================
...
function markov_image(img; n_states=64)
    ...
    out = Array{RGB{Float64}}(undef, oh, ow)
...
# =============================================================================
# 14. Dynamical maps (grayscale density)
# =============================================================================
...
function domain_warp_gray(gray_img, field_img; strength=0.15)
    ...
    for py in 1:oh, px in 1:ow
        ...
        du = amp * dx[fy, fx]
        dv = amp * dy[fy, fx]
        u_warp = clamp(u + du, 0, 1)
        v_warp = clamp(v + dv, 0, 1)
        sx = u_warp * (ow - 1) + 1
        sy = v_warp * (oh - 1) + 1
...
# =============================================================================
# 17. FLUID MOTION WARP (NEW)
# =============================================================================

"""
Apply a fluid, random motion warp to a grayscale image.
strength ∈ [0,1] controls the maximum displacement.
seed ensures reproducibility for the same base image.
"""
function motion_warp(gray_img; strength=MOTION_STRENGTH, seed=0)
    h, w = size(gray_img)
    rng = MersenneTwister(seed)
    # Random frequencies and phases for smooth flow field
    freq_x = rand(rng, 3:7)
    freq_y = rand(rng, 3:7)
    phase_x = rand(rng, length(freq_x)) * 2π
    phase_y = rand(rng, length(freq_y)) * 2π
    amp_x = rand(rng, length(freq_x)) .* 0.5 .+ 0.5
    amp_y = rand(rng, length(freq_y)) .* 0.5 .+ 0.5

    dx = zeros(Float64, h, w)
    dy = zeros(Float64, h, w)
    for py in 1:h, px in 1:w
        u = (px-1)/(w-1)
        v = (py-1)/(h-1)
        sum_x = 0.0; sum_y = 0.0
        for i in 1:length(freq_x)
            sum_x += amp_x[i] * sin(2π * freq_x[i] * u + phase_x[i])
            sum_y += amp_y[i] * cos(2π * freq_y[i] * v + phase_y[i])
        end
        dx[py,px] = sum_x / length(freq_x)
        dy[py,px] = sum_y / length(freq_y)
    end

    # Scale to pixel displacement
    max_disp = strength * min(h, w)
    max_abs_dx = max(maximum(abs, dx), 1e-6)
    max_abs_dy = max(maximum(abs, dy), 1e-6)
    dx .*= max_disp / max_abs_dx
    dy .*= max_disp / max_abs_dy
    ...
    for py in 1:h, px in 1:w
        src_x = px - dx[py, px]
        src_y = py - dy[py, px]
        ix_low = floor(Int, src_x); ix_high = ix_low + 1
        iy_low = floor(Int, src_y); iy_high = iy_low + 1
        wx = src_x - ix_low; wy = src_y - iy_low
        ix_low = clamp(ix_low, 1, w); ix_high = clamp(ix_high, 1, w)
        iy_low = clamp(iy_low, 1, h); iy_high = clamp(iy_high, 1, h)
        v11 = gray_img[iy_low, ix_low]; v21 = gray_img[iy_low, ix_high]
        v12 = gray_img[iy_high, ix_low]; v22 = gray_img[iy_high, ix_high]
        top = (1-wx)*v11 + wx*v21
        bot = (1-wx)*v12 + wx*v22
...
# =============================================================================
# 18. Mathematical art loop – only PCA images, with motion + FFT
# =============================================================================

println("\n===== IMAGE-CONDITIONED MATHEMATICAL ART (PCA + MOTION + FFT) =====")
ml_files = filter(f -> startswith(f, "ml_pca_"), image_files(ML_OUTPUT))

if isempty(ml_files)
    println("No PCA images found in $ML_OUTPUT – skipping mathematical art.")
else
    for f in ml_files
        println("\nGenerating from $f")
        ...
        # Modified processing pipeline: motion warp → FFT → duotone
        function process_gray(gray)
            # Unique motion seed per image to vary the flow
            warped = motion_warp(gray, strength=MOTION_STRENGTH,
                                 seed=image_hash_seed(img_proc, base_seed=999))
        ...
        for step in 1:4
            ...
            safe_save(joinpath(FRACT_OUTPUT, "fract_$(base)_morph_js_$(step).png"),
                      process_gray(morph_gray(julia_gray, ship_gray, a)))
            safe_save(joinpath(FRACT_OUTPUT, "fract_$(base)_morph_jc_$(step).png"),
                      process_gray(morph_gray(julia_gray, chirikov_gray, a)))
            safe_save(joinpath(FRACT_OUTPUT, "fract_$(base)_morph_ch_$(step).png"),
                      process_gray(morph_gray(chirikov_gray, henon_gray, a)))
        end
...
Post‑modern art (motion):  $FRACT_OUTPUT
===============================================================================
""")